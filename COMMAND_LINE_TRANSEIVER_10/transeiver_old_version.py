...
class Transeiver:
	...
	@staticmethod
	def receiving(ser,queue):         #gets the receiving messages in a parallel process # this gets put under cases
		cant_decode = False
		while True:
...
class MasterTranseiver(Transeiver):    #some networks have properties, some with changing address or those that are not available #central ID for node might be needed
	...
	def __search(self, mode = 0):   #in theory this protocol might work
		...
		while time.monotonic() < end_time:
			self.serial_port.write(bytes(Transeiver.MESSAGING + "ADDRESS-" + self.TXaddress + Transeiver.FLUSH,encoding = 'utf-8')) #maybe this can work if using other pipes
			print("sent")
			time.sleep(0.2)
			if not self.receive_queue.empty():
				message = self.receive_queue.get()
				print("received")
				if "ADDRESS-" in message and message != '' and message != "<<<success>>>" and message not in self.neighboring_nodes: #the code should stop using this success protocol
					print("added")
