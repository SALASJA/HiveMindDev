...
void process_uart_input(uint8_t * data_buffer, uint8_t * tx_address, uint8_t * rx_address, uint8_t * success_mode){
	switch(data_buffer[0]){
		...
		case '1': setAddress(tx_address,data_buffer, TRUE);
				  break;
		case '2': setAddress(rx_address,data_buffer, FALSE);	
				  break;
...
void process_recieved(uint8_t * receive_buffer, uint8_t * tx_address, uint8_t * rx_address, uint8_t * success_mode, uint8_t * receiving){
...
void setAddress(uint8_t * address_buffer, uint8_t * new_address, uint8_t mode){
	for(uint8_t i = 0; i < 5; i++){
	...
	if(mode){
		...
		nrf24_rx_address(address_buffer);
	}
}
