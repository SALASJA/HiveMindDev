...
uint8_t * Framebuffer::getCharacterBitmap(uint8_t c){
	if ('a' <= c && c <= 'z'){
		c = c - 32;
	}
	c = c - 'A';
	return (uint8_t *)this->text_table[c];
}

void Framebuffer::drawText(const char * text, uint8_t pos_x, uint8_t pos_y){
	char c;
	uint8_t count = 0;
	uint8_t new_line = 0;
	for(int j = 0; text[j] != '\0'; j++){
		...
		uint8_t * bitmap = this->getCharacterBitmap(c);
		for (int i = 0; i < 5; i++) {
			for (int p = 0; p < 8; p++) {
				if ((bitmap[i] >> p) & 0x1) {
					this->drawPixel(7 - p + pos_x + count * 7, i+pos_y + new_line * 10);
				}
			}
		}
		count++;
